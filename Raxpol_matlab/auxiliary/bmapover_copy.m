...
%
function [VIS] = bmapover_copy(axes_handler_set,flags,origin,substate,data_type)
...
origin = {-97.4639,35.2368,'PX1000','OK'};
...
MAP(11).lab = {'OU-PRIME','NWRT','KCYR','KRSP','KSAO','KLWE',...
...
if ismember(origin,{'KCRI','PAR','KOUN'}), origin = 'OU-PRIME'; end
...
%         else
%             MAP(idx).x = MAP(idx).x(mask)+19.48*cos(2*pi/360*31.77);
%             MAP(idx).y = MAP(idx).y(mask)+19.48*sin(2*pi/360*31.77);
%          end
        MAP(idx).x = MAP(idx).x(mask);
        MAP(idx).y = MAP(idx).y(mask);
