...
%and CFradial files from RaXPol.

%In the directory where this script is located, there needs to be a 

% clear; close all; 
addpath auxiliary; %I put the getmenu files in a subfolder auxilary

dir_name=uigetdir; %gets the name of the directory
%dir_name='/Volumes/BrandonData/Wakimoto_Data/';

%%%%% MODIFY THIS TO MATCH THE DIRECTORY WHERE THE SCRIPT IS %%%%%%
%base_dir='/Users/brandoncohen/Documents/0_OU/Actual_RaXPol_Viewer/0_Research';
base_dir='/Users/brandoncohen/Documents/0_OU/0_research/Actual_RaXPol_Viewer/0_Research'
%base_dir='/Users/brandoncohen/Dropbox/Mac/Documents/0_OU/0_research/Actual_RaxPol_Viewer/0_Research';
% base_dir = '/Users/brandoncohen/Dropbox/Mac/Downloads/output/20130519';
%base_dir='/Users/brandoncohen/Documents/0_OU/0_Research'; %This is your main directory where this script is run
%base_dir='/Volumes/BrandonData/0_Research';
%base_dir='/Volumes/BrandonData/20110524_unfolded/ConvertedData/20110524';
if(~exist([base_dir '/figs/'],'dir'))
    mkdir([base_dir '/figs/']);
end

data_type=input('Enter data type: 1) .mat, 2) RaXPol/CFRadial ');
ouprime_flag=false;
st_video=true;
preload_file=true; %Preload directory structure to save time

rthres=-1; %Radius for gathering data for TVSs/TDSs (-1 to get user input)
...
if(big_monitor)
    plot_choice=[1 2 3 4 6]; 
%     plot_choice=[1 2 3 4 6 7]; % original
...
vr_lims=[-25 25]; zdr_lims=[-3 5]; vr_lims2=[-60 60]; %vr_lims2=[-60 60];
...
pos1=[1+adjust_pos(1) 1 winsize]; pos2=[winsize(1)+1+adjust_pos(1) 1 winsize]; pos6=[1+adjust_pos(1) winsize(1)+1+adjust_pos(2) 600 120];; pos8=[winsize(1)+adjust_pos(1) winsize(1)+110+adjust_pos(2) 260 100]; pos7=pos8;
...
axis_label_size=26; options=8; is_running=true; title_size=26;  zoom_factor_x=3; zoom_factor_y=3; 
...
cd(dir_name)
    if(data_type==1)
        ...
        for idx=1:max(size(files))
            ...
            ele_save(idx)=str2num(file_tmp(file_name_length-5:file_name_length-4)); 
        end
    elseif(data_type==2)
        ...
        if(exist('dir_info.mat','file') && preload_file)
            ...
            for idx=1:max(size(files))
                file_tmp=files(idx).name;
                time_tmp=ncreadatt(file_tmp,'/','start_datetime');
...
while(is_running)
    ...
    if(isfield(data,'roll'))
        ...
        %Headings appear to be switched between deployments, adhoc fix
        if(data.heading<85)
...
%     if(radar_choice<3)
    ...
    if(~isfield(data,'Station'))
        if(isfield(data,'roll') || isfield(data,'heading'))
            ...
            %data.Station=input('Input radar name: ','s');
            data.Station='RaXPol';
        end
    end
    ...
    for pdx=1:num_loops
        ...
        if(~fig_exists)
            ...
            if(plot_choice(pdx)==1)
                if(att_corr_flag)
                    data_tmp2=double(data.Z_corr);
                else
                    data_tmp2=double(data.Z_H(1:size(data.vr_h,1),1:size(data.vr_h,2)));
                end
                p1(pdx)=pcolor(xx,yy,data_tmp2);
                %p1(pdx)=pcolor(xx(:,191:247),yy(:,191:247),data_tmp2(:,191:247));
                %272:329 for 23:24:11z
                %191:247 for 23:24:30z
                set(p1(pdx),'EdgeColor','none');
                colormap(cmap3)
                shading interp
                caxis(clims3)
                title([num2str(round(ele*10)/10) '^{\circ} Reflectivity at ' time_choice ' UTC'],'FontSize',title_size,'Interpreter','tex','HorizontalAlignment','Center')
            ...
            elseif(plot_choice(pdx)==2)
                ...
                if(exist('xx2','var'))
                    ...
                    %p1(pdx)=pcolor(xx(:,191:247),yy(:,191:247),data_tmp(:,191:247));
                end
                colormap(cmap5)
%                 h = colorbar;
%                 title(h, 'm s^{-1}');
...
%                 colormap(cmap3)
            ...
            if(add_map)
                if(KOUN_adjust_flag)
                    ...
                    bmapover(gca,bmap_flags,{data.lon,data.lat,'','OK'})
                end
            end
        else
            figure(pdx)
